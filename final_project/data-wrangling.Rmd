...
### save this data
write.csv(new_cases_data, here('data', 'processed_data','ny_times_with_new_cases.csv'))
```
...
new_cases_data <- new_cases_data %>%
  mutate(report_date = as.Date(date)) %>%
  select(-date) %>%
  rename(date = report_date)

joined_data <- merge(x = new_cases_data, y = mask_mandate_data, by='state', all=TRUE) %>% select(-X)
write.csv(joined_data, here('data', 
                            'processed_data',
                            'cases_data_plus_mandates.csv'))
```
